...
const knownFailingPages: string[] = ['error-frame.html', 'error-occurred.html'];

describe('integration/full/error-occurred', () => {
  if (pages.length > 0) {
    it.each(pages)('%s', { timeout: 60_000, retry: 1 }, async page => {
      const fixture = `/test/integration/full/error-occurred/${page}`;
