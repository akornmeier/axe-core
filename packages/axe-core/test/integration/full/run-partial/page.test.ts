...
const pages: string[] = ['initiator.html', 'page-level.html'];
...
const knownFailingPages: string[] = [
  'after-method.html',
  'context-size-focusable.html'
];

describe('integration/full/run-partial', () => {
  if (pages.length > 0) {
    it.each(pages)('%s', { timeout: 60_000, retry: 1 }, async page => {
      const fixture = `/test/integration/full/run-partial/${page}`;
