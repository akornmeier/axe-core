...
const pages: string[] = ['shadow-dom.html'];
...
const knownFailingPages: string[] = ['context.html'];

describe('integration/full/context', () => {
  if (pages.length > 0) {
    it.each(pages)('%s', { timeout: 60_000, retry: 1 }, async page => {
      const fixture = `/test/integration/full/context/${page}`;
