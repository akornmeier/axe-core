...
const pages: string[] = ['incomplete.html', 'passes.html', 'violations.html'];

describe('integration/full/css-orientation-lock', () => {
  if (pages.length > 0) {
    it.each(pages)('%s', { timeout: 60_000, retry: 1 }, async page => {
      const fixture = `/test/integration/full/css-orientation-lock/${page}`;
