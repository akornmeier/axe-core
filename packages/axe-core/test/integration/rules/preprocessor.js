...
const fs = require('fs');
const extRegex = /\.json$/;
const template = fs.readFileSync(path.join(__dirname, 'runner.js'), 'utf-8');
 ...
 */
const createIntegrationPreprocessor = function (logger) {
  ...
  return function (content, file, done) {
    try {
      ...
      const htmlpath = file.originalPath.replace(extRegex, '.html');
      ...
      const result = template.replace('{}; /*tests*/', JSON.stringify(test));
