...
describe('aria-required-children', () => {
  ...
  it('should detect missing sole required child', () => {
    ...
    expect(requiredChildrenCheck.apply(checkContext, params as any)).toBe(
      false
    );
    expect(checkContext._data).toEqual(['listitem']);
  });
  ...
  it('should detect multiple missing required children when one required', () => {
    ...
    expect(checkContext._data).toEqual(['rowgroup', 'row']);
  });
  ...
  it('should pass all existing required children when all required', () => {
    ...
    expect(requiredChildrenCheck.apply(checkContext, params as any)).toBe(true);
  });
  ...
  it('should return undefined when element is empty and is in reviewEmpty options', () => {
    ...
    expect(
      requiredChildrenCheck.apply(checkContext, params as any)
    ).toBeUndefined();
  });
  ...
  it('should pass when missing required children but aria-busy', () => {
    ...
    expect(checkContext._data).toEqual({ messageKey: 'aria-busy' });
  });
  ...
  it('should fail when list has intermediate child with role that is not a required role', () => {
    ...
    expect(checkContext._relatedNodes).toEqual([unallowed]);
  });
  ...
  it('should fail when nested child with role row does not have required child role cell', () => {
    ...
    // Chai's `includeMembers` → Vitest's `arrayContaining`. Codemod gap; one-off.
    expect(checkContext._data).toEqual(expect.arrayContaining(['cell']));
  });
