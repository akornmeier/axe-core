...
const isOnScreenEvaluateESM = getCheckEvaluateESM(isOnScreenEvaluate);
describe('is-on-screen', () => {
  it('should return true for visible elements', () => {
    const vNode = queryFixture('<div id="target">elm</div>');

    expect(isOnScreenEvaluateESM(vNode)).toBe(true);
  });
  ...
  it('should return false for display:none elements', () => {
    ...
    expect(isOnScreenEvaluateESM(vNode)).toBe(false);
  });
