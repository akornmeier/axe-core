...
function autocompleteMatches(
  ...
  if (
    nodeName === 'input' &&
    excludedInputTypes.includes(virtualNode.props.type as string)
  ) {
    return false;
  }
  ...
  if (tabIndex !== null && tabIndex < 0 && virtualNode.hasAttr('role')) {
    const roleDef = (standards as any).ariaRoles[role!];
