...
const definitions: ResultDefinition[] = [
  {
    name: 'NA',
    value: 'inapplicable',
    priority: 0,
...
const constants: Record<string, unknown> & {
  helpUrlBase: string;
  gridSize: number;
  selectorSimilarFilterLimit: number;
  results: string[];
  resultGroups: string[];
  resultGroupMap: Record<string, string>;
  impact: readonly string[];
  preload: Readonly<{ assets: string[]; timeout: number }>;
  allOrigins: string;
  sameOrigin: string;
  serializableErrorProps: readonly string[];
} = {
