...
function preloadMedia({
  ...
  const mediaVirtualNodes = querySelectorAllFilter(
    ...
    (node: unknown) => {
      const { actualNode } = node as { actualNode: HTMLMediaElement };
      /**
  ...
  return Promise.all(
    mediaVirtualNodes.map((node: unknown) =>
      isMediaElementReady(
        (node as Record<string, unknown>).actualNode as HTMLMediaElement
      )
    )
  );
}
 ...
 * @returns {Promise}
 */
function isMediaElementReady(elm: HTMLMediaElement): Promise<HTMLMediaElement> {
