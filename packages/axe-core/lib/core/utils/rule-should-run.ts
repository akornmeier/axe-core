...
function matchTags(
  ...
  runOnly: Record<string, unknown> | unknown[]
): boolean {
  let include: unknown[];
  let exclude: unknown[];
  const defaultExclude: unknown[] =
    // @ts-expect-error - axe is a global variable
    axe._audit && axe._audit.tagExclude ? axe._audit.tagExclude : [];

  // normalize include/exclude
  if (
    (runOnly as Record<string, unknown>).hasOwnProperty?.('include') ||
    (runOnly as Record<string, unknown>).hasOwnProperty?.('exclude')
  ) {
    // Wrap include and exclude if it's not already an array
    include = ((runOnly as Record<string, unknown>).include as unknown[]) || [];
    include = Array.isArray(include) ? include : [include];

    exclude = ((runOnly as Record<string, unknown>).exclude as unknown[]) || [];
  ...
  const matching = include.some(tag => {
    return (rule.tags as unknown[]).indexOf(tag) !== -1;
  });
 ...
 * @return {Boolean}
 */
function ruleShouldRun(
  rule: Record<string, unknown>,
  context: Record<string, unknown>,
  options: Record<string, unknown>
): boolean {
  const runOnly = (options.runOnly || {}) as Record<string, unknown>;
  const ruleOptions = ((options.rules || {}) as Record<string, unknown>)[
    rule.id as string
  ...
  if (rule.pageLevel && !context.page) {
    ...
    return (runOnly.values as unknown[]).indexOf(rule.id) !== -1;
    ...
    return matchTags(rule, runOnly.values as Record<string, unknown>);
