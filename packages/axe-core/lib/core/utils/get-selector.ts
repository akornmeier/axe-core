 ...
 * @return {String}
 */
function getAttributeNameValue(node: Element, at: Attr): string {
  const name = at.name;
  let atnv: string;

  if (name.indexOf('href') !== -1 || name.indexOf('src') !== -1) {
    const friendly = getFriendlyUriEnd(node.getAttribute(name) ?? undefined);
    if (friendly) {
...
function countSort(a: FeatureItem, b: FeatureItem): number {
 ...
 */
function filterAttributes(at: Attr): boolean {
  return (
 ...
 */
export function getSelectorData(domTree: unknown): SelectorData {
  ...
  while (currentLevel.length) {
    const current = currentLevel.pop() as Record<string, unknown>;
    const node = current.actualNode as Element;

    if (!!(node as unknown as Record<string, unknown>).querySelectorAll) {
      // ignore #text nodes
      ...
      if (node.hasAttributes()) {
        Array.from(getNodeAttributes(node) as unknown as Iterable<Attr>)
    ...
    if ((current.children as unknown[]).length) {
      // "recurse"
      stack.push(currentLevel);
      currentLevel = (current.children as unknown[]).slice();
    }
 ...
 */
function uncommonClasses(
  node: Element,
  selectorData: SelectorData
): FeatureItem[] {
  // eslint no-loop-func:false
  const retVal: FeatureItem[] = [];
  ...
  if (node.classList) {
    Array.from(node.classList).forEach(cl => {
      const ind = escapeSelector(cl);
      if ((classData[ind] ?? 0) < (tagData[node.nodeName] ?? 0)) {
        retVal.push({
          name: ind,
          count: classData[ind] ?? 0,
          species: 'class'
        });
      }
    });
...
function getNthChildString(elm: Element, selector: string): string {
  const siblings =
    (elm.parentNode &&
      Array.from((elm.parentNode as Element).children || '')) ||
    [];
...
function getElmId(elm: Element): string | undefined {
  ...
  if (
    ...
    (doc as Document).querySelectorAll(id).length === 1
  ) {
    return id;
  }
}
  ...
  if (node.hasAttributes()) {
    Array.from(getNodeAttributes(node) as unknown as Iterable<Attr>)
      ...
      .forEach(at => {
        ...
        if (atnv && (attData[atnv] ?? 0) < (tagData[node.nodeName] ?? 0)) {
          retVal.push({
            name: atnv,
            count: attData[atnv] ?? 0,
            species: 'attribute'
          });
        }
      });
...
function getThreeLeastCommonFeatures(
  elm: Element,
  selectorData: SelectorData
): string {
  let selector = '';
    ...
    if (
      ...
      similar!.length >
        ((constants as Record<string, unknown>)
          .selectorSimilarFilterLimit as number)
    ) {
      similar = findSimilar(doc, selector) as Element[];
    } else {
 ...
 */
function getSelector(elm: Element, options?: Record<string, unknown>): string {
  return getShadowSelector(generateSelector, elm, options) as string;
}
...
export default memoize(
  getSelector as (...args: unknown[]) => unknown
) as typeof getSelector;
...
const findSimilar = memoize(((
  doc: Document | DocumentFragment,
  selector: string
) => Array.from(doc.querySelectorAll(selector))) as (
  ...args: unknown[]
) => unknown) as (
  doc: Document | DocumentFragment,
  selector: string
