...
let uuid: typeof v4 & {
  ...
  BufferClass: typeof BufferClass;
};
...
const _hexToByte: Record<string, number> = {};
for (let i = 0; i < 256; i++) {
...
function parse(s: string, buf?: number[], offset?: number): number[] {
  let i = (buf && offset) || 0,
    ii = 0;
...
function unparse(buf: number[] | Uint8Array, offset?: number): string {
  let i = offset || 0,
    bth = _byteToHex;
  return (
    bth[buf[i++]!]! +
...
// See https://github.com/broofa/node-uuid for API details
function v4(
  options?: UuidOptions | string | null,
  buf?: number[],
  offset?: number
): string | number[] {
  ...
  if (typeof options == 'string') {
    buf =
      options == 'binary'
        ? (new BufferClass!(16) as unknown as number[])
        : undefined;
    options = null;
  }
  options = options || {};
