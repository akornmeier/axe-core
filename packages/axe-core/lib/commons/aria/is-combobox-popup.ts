...
interface ComboboxPopupOptions {
  popupRoles?: string[];
}
...
export default function isComboboxPopup(
  virtualNode: any,
  { popupRoles }: ComboboxPopupOptions = {}
): boolean {
...
const isCombobox = (node: any): boolean => node && getRole(node) === 'combobox';

function nearestParentWithRole(vNode: any): any {
