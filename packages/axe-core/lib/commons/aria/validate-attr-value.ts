 ...
 * @return {Boolean}
 */
function validateAttrValue(vNode: any, attr: string): boolean | undefined {
  vNode = vNode instanceof AbstractVirtuaNode ? vNode : getNodeFromTree(vNode);

  let matches: RegExpMatchArray | null;
  let list: string[];
  const value: string = vNode.attr(attr);
  ...
  switch (attrInfo.type) {
    ...
    case 'nmtokens':
      ...
      return list.reduce((result: boolean, token: string) => {
    ...
    case 'idrefs':
      return idrefs(vNode, attr).some((node: unknown) => !!node);
