...
lookupTable.elementsAllowedNoRole = [
  ...
  },
  {
    nodeName: 'select',
    condition: (vNode: any) => {
      if (!(vNode instanceof (axe as any).AbstractVirtualNode)) {
        vNode = (axe as any).utils.getNodeFromTree(vNode);
      }
...
lookupTable.evaluateRoleForElement = {
  ...
  AREA: ({ node }: { node: any }) => !node.href,
  BUTTON: ({ node, role, out }: { node: any; role: any; out: any }) => {
    if (node.getAttribute('type') === 'menu') {
      return role === 'menuitem';
    }
    return out;
  },
  IMG: ({ node, role, out }: { node: any; role: any; out: any }) => {
    switch (node.alt) {
      case null:
        return out;
      case '':
  ...
  INPUT: ({ node, role, out }: { node: any; role: any; out: any }) => {
    switch (node.type) {
      case 'button':
      case 'image':
        return out;
  ...
  LI: ({ node, out }: { node: any; out: any }) => {
    const hasImplicitListitemRole = (axe as any).utils.matchesSelector(
      node,
      'ol li, ul li'
    );
  ...
  OPTION: ({ node }: { node: any }) => {
    const withinOptionList = (axe as any).utils.matchesSelector(
      node,
      'select > option, datalist > option, optgroup > option'
    );
    return !withinOptionList;
  },
  SELECT: ({ node, role }: { node: any; role: any }) =>
    !node.multiple && node.size <= 1 && role === 'menu',
  SVG: ({ node, out }: { node: any; out: any }) => {
