...
function getRoleSegments(vNode: any): string[] {
  let roles: string[] = [];
  if (!vNode) {
    return roles;
  }
  ...
  return roles.filter((role: string) => isValidRole(role));
}
 ...
 */
function getElementUnallowedRoles(
  node: any,
  ...
  const implicitRole =
    getImplicitRole(vNode) || landmarkRoles[nodeName] || null;

  const roleSegments = getRoleSegments(vNode);
  return roleSegments.filter((role: string) => {
