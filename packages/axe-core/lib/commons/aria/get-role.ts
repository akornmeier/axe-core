...
interface RoleOptions extends ExplicitRoleOptions {
  noImplicit?: boolean;
...
function getInheritedRole(
  vNode: any,
  explicitRoleOptions: ExplicitRoleOptions
): string | null {
...
function hasConflictResolution(vNode: any): boolean {
  const hasGlobalAria = getGlobalAriaAttrs().some((attr: string) =>
    vNode.hasAttr(attr)
  );
