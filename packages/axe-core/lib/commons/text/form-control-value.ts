...
export const controlValueRoles: string[] = [
  'textbox',
  'progressbar',
  'scrollbar',
  'slider',
...
export const formControlValueMethods: Record<
  string,
  (node: unknown, context?: AccessibleTextContext) => string
> = {
  nativeTextboxValue,
  nativeSelectValue,
  ariaTextboxValue,
...
function formControlValue(
  ...
  if (
    ...
    (unsupportedRoles as readonly string[]).includes(role)
  ) {
    return '';
  }
  ...
  const valueString = valueMethods.reduce((accName: string, step) => {
 ...
 * @return {string} The calculated value
 */
function nativeTextboxValue(node: unknown): string {
 ...
 * @return {string} The calculated value
 */
function nativeSelectValue(node: unknown): string {
  ...
  const selectedOptions = options.filter(
    (option: any) => option.props.selected
  );
  ...
  return (
    selectedOptions
      .map((option: unknown) => visibleVirtual(option))
      .join(' ') || ''
  );
}
 ...
 * @return {string} The calculated value
 */
function ariaTextboxValue(node: unknown): string {
...
function ariaComboboxValue(
  ...
  const listbox = getOwnedVirtual(vNode).filter(
    (elm: unknown) => getRole(elm) === 'listbox'
  )[0];
 ...
 * @return {string} The calculated value
 */
function ariaRangeValue(node: unknown): string {
  const { vNode }: { vNode: any } = nodeLookup(node);
