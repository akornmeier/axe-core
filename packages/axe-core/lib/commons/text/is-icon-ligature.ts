...
export default function isIconLigature(
  textVNode: unknown,
  differenceThreshold: number = 0.15,
  occurrenceThreshold: number = 3
): boolean {
  /**
   ...
   * └─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┴─┘
   */
  const nodeValue = (textVNode as any).actualNode.nodeValue.trim();
  ...
  // as a ligature.
  const fonts = cache.get('fonts', () => ({})) as Record<
    string,
    { occurrences: number; numLigatures: number }
  >;
  const style = window.getComputedStyle((textVNode as any).parent.actualNode);
  ...
  const expectedWidth = nodeValue
    .split('')
    .reduce((totalWidth: number, char: string) => {
