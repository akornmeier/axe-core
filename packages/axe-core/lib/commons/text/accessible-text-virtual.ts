...
export interface AccessibleTextContext {
  startNode?: unknown;
...
function accessibleTextVirtual(
  ...
  type ComputationStep = (
    vNode: unknown,
    ctx?: AccessibleTextContext
  ) => string;
  const computationSteps: ComputationStep[] = [
    arialabelledbyText as ComputationStep, // Step 2B.1
    arialabelText as ComputationStep, // Step 2C
    ...
    titleText as ComputationStep // Step 2I
  ];

  // Find the first step that returns a non-empty string
  const accessibleName = computationSteps.reduce((accName: string, step) => {
  ...
  if (context.debug) {
    (axe as any).log(
      accessibleName || '{empty-value}',
      (virtualNode as any).actualNode,
      context
    );
  }
 ...
 */
function textNodeValue(virtualNode: unknown): string {
  if ((virtualNode as any).props.nodeType !== 3) {
    return '';
  }
  return (virtualNode as any).props.nodeValue;
}
 ...
 * @return {Boolean}
 */
function shouldIgnoreIconLigature(
  virtualNode: unknown,
  context: AccessibleTextContext
): boolean {
  ...
  if ((virtualNode as any).props.nodeType !== 3 || !ignoreIconLigature) {
    return false;
  }
