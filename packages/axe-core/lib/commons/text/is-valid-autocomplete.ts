...
export const autocomplete: AutocompleteTerms = {
  stateTerms: ['on', 'off'],
  standaloneTerms: [
    'name',
    'honorific-prefix',
...
export interface IsValidAutocompleteOptions {
  looseTyped?: boolean;
  stateTerms?: string[];
  locations?: string[];
  qualifiers?: string[];
...
function isValidAutocomplete(
  ...
  if (!looseTyped) {
    ...
    if (autocompleteTerms[0] && locations.includes(autocompleteTerms[0])) {
      autocompleteTerms.shift();
    }

    if (autocompleteTerms[0] && qualifiers.includes(autocompleteTerms[0])) {
      autocompleteTerms.shift();
