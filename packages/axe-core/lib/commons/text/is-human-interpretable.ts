 ...
 */
function isHumanInterpretable(str: string): number {
  /**
   * Steps:
...
function isNonDigitCharacter(str: string): boolean {
  return str.length === 1 && !!str.match(/\D/);
}

function isSymbolicText(str: string): boolean {
  const symbolicText = [
...
function isUnicodeOrPunctuation(str: string): boolean {
