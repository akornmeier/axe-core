 ...
 */
export default function splitRects(
  outerRect: DOMRect,
  overlapRects: DOMRect[]
): DOMRect[] {
  let uniqueRects: DOMRect[] = [outerRect];
  for (const overlapRect of overlapRects) {
    uniqueRects = uniqueRects.reduce<DOMRect[]>((rects, inputRect) => {
...
function splitRect(inputRect: DOMRect, clipRect: DOMRect): DOMRect[] {
...
function computeRect(baseRect: RectEdges): DOMRect {
...
function isEnclosedRect(rectA: RectEdges, rectB: RectEdges): boolean {
  return (
    rectA.top >= rectB.top &&
