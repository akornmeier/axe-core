 ...
 * @param {Element} node
 */
const getVisibleChildTextRects: (node: Element) => DOMRect[] = memoize(
  function getVisibleChildTextRectsMemoized(node: Element): DOMRect[] {
    ...
    node.childNodes.forEach((textNode: ChildNode) => {
      if (
        textNode.nodeType !== 3 ||
        sanitize(textNode.nodeValue ?? '') === ''
      ) {
        return;
      }
...
function getContentRects(node: ChildNode): DOMRect[] {
 ...
 */
function isOutsideNodeBounds(rects: DOMRect[], nodeRect: DOMRect): boolean {
  return rects.some((rect: DOMRect) => {
 ...
 * ancestors
 */
function filterHiddenRects(
  contentRects: DOMRect[],
  overflowHiddenNodes: any[]
): DOMRect[] {
  const visibleRects: DOMRect[] = [];
  contentRects.forEach((contentRect: DOMRect) => {
