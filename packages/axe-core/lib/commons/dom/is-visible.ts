...
function isClipped(style: CSSStyleDeclaration): boolean {
  ...
  if (matchesClip && matchesClip.length === 5) {
    ...
    if (['fixed', 'absolute'].includes(position)) {
      return (
        (matchesClip[3] as any) - (matchesClip[1] as any) <= 0 &&
        (matchesClip[2] as any) - (matchesClip[4] as any) <= 0
      );
    }
  }
  if (matchesClipPath) {
...
function isAreaVisible(
  ...
  return refs.some((ref: unknown) =>
    isVisible(
      (ref as { actualNode: Element }).actualNode,
      screenReader,
      recursed
    )
  );
...
function isVisible(
  ...
  const vNode: any =
    el instanceof AbstractVirtualNode ? el : getNodeFromTree(el);
  el = vNode ? vNode.actualNode : el;

  const cacheName = '_isVisible' + (screenReader ? 'ScreenReader' : '');
  const { DOCUMENT_NODE, DOCUMENT_FRAGMENT_NODE } =
    window.Node ?? ({} as Record<string, number>);
  const nodeType = vNode ? vNode.props.nodeType : el.nodeType;
  const nodeName = vNode ? vNode.props.nodeName : el.nodeName.toLowerCase();

  if (vNode && typeof (vNode as any)[cacheName] !== 'undefined') {
    return (vNode as any)[cacheName];
  }
  ...
  if (!el) {
    const parent = (vNode as any).parent;
    let visible = true;
    if (parent) {
    ...
    if (vNode) {
      (vNode as any)[cacheName] = visible;
    }
