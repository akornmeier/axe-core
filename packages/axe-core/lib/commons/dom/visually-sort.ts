 ...
 * @param {VirtualNode}
 */
export default function visuallySort(a: any, b: any): number {
  /*eslint no-bitwise: 0 */
  ...
  let aNode: Node = a.actualNode;
  let bNode: Node = b.actualNode;
  ...
  if (
    (aNode as Element).getRootNode &&
    (aNode as Element).getRootNode() !== (bNode as Element).getRootNode()
  ) {
    // keep track of all parent hosts and find the one both nodes share
    const boundaries: Boundary[] = [];
    while (aNode) {
      boundaries.push({
        root: (aNode as Element).getRootNode(),
        node: aNode
      });
      aNode = ((aNode as Element).getRootNode() as ShadowRoot).host;
    }
    ...
    while (
      ...
      !boundaries.find(
        (boundary: Boundary) =>
          boundary.root === (bNode as Element).getRootNode()
      )
    ) {
      bNode = ((bNode as Element).getRootNode() as ShadowRoot).host;
    }
 ...
 * @return {Number}
 */
function getPositionOrder(vNode: any): number {
 ...
 */
function isFloated(vNode: any): boolean {
  if (!vNode) {
    return false;
  }
