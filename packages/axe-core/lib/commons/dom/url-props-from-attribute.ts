 ...
 * @returns {Object}
 */
function urlPropsFromAttribute(
  node: Element,
  attribute: string
): UrlProps | undefined {
  ...
  let parser: HTMLAnchorElement | HTMLAreaElement | Element = node;
   ...
   */
  if (
    !['A', 'AREA'].includes(nodeName) ||
    (node as SVGElement).ownerSVGElement
  ) {
    parser = document.createElement('a');
    (parser as HTMLAnchorElement).href = node.getAttribute(attribute)!;
  }

  /**
   * Curate `https` and `ftps` to `http` and `ftp` as they will resolve to same resource
   */
  const protocol = [`https:`, `ftps:`].includes(
    (parser as HTMLAnchorElement).protocol
  )
    ? (parser as HTMLAnchorElement).protocol.replace(/s:$/, ':')
    : (parser as HTMLAnchorElement).protocol;
   ...
   */
  const parserPathname = /^\//.test((parser as HTMLAnchorElement).pathname)
    ? (parser as HTMLAnchorElement).pathname
    : `/${(parser as HTMLAnchorElement).pathname}`;
  const { pathname, filename } = getPathnameOrFilename(parserPathname);

  return {
    protocol,
    hostname: (parser as HTMLAnchorElement).hostname,
    port: getPort((parser as HTMLAnchorElement).port),
    pathname: /\/$/.test(pathname) ? pathname : `${pathname}/`,
    search: getSearchPairs((parser as HTMLAnchorElement).search),
    hash: getHashRoute((parser as HTMLAnchorElement).hash),
    filename
  };
}
 ...
 */
function getPathnameOrFilename(pathname: string): PathnameResult {
 ...
 * @returns {Object}
 */
function getSearchPairs(searchStr: string): Record<string, string> {
 ...
 * @returns {String}
 */
function getHashRoute(hash: string): string {
  if (!hash) {
    return ``;
  }
