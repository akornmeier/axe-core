 ...
 * @return {Boolean}
 */
export function nativelyHidden(vNode: any): boolean {
 ...
 * @return {Boolean}
 */
export function displayHidden(vNode: any): boolean {
 ...
 * @return {Boolean}
 */
export function ariaHidden(vNode: any): boolean {
 ...
 * @return {Boolean}
 */
export function opacityHidden(vNode: any): boolean {
 ...
 * @return {Boolean}
 */
export function scrollHidden(vNode: any): boolean {
 ...
 * @return {Boolean}
 */
export function clipHidden(vNode: any): boolean {
...
export function areaHidden(
  vNode: any,
  visibleFunction: (vNode: any, isAncestor?: boolean) => boolean
): boolean {
  /**
   * Note:
  ...
  const mapElName = (mapEl as any).attr('name');
  if (!mapElName) {
    return true;
  }
  ...
  if (!mapElRootNode || (mapElRootNode as Node).nodeType !== 9) {
    return true;
  }
  ...
  return refs.some((ref: any) => !visibleFunction(ref));
}

/**
 * Determine if element inside `details` is hidden
 */
export function detailsHidden(vNode: any): boolean {
  ...
  if (vNode.props.nodeName === 'summary') {
    const firstSummary = vNode.parent.children.find(
      (node: any) => node.props.nodeName === 'summary'
    );
...
function hasPositionedAncestorBetween(child: any, ancestor: any): boolean {
