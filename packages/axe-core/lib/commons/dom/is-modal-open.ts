 ...
 */
function isModalOpen(options?: IsModalOpenOptions): boolean | undefined {
  options = options || {};
  ...
  if (cache.get('isModalOpen')) {
    return cache.get('isModalOpen') as boolean | undefined;
  }
  ...
  const stacks: Element[][] = points.map(point => {
