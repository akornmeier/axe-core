 ...
 * @returns boolean
 */
export function hasChildTextNodes(elm: any): boolean {
  if (hiddenTextElms.includes(elm.props.nodeName)) {
    return false;
  }
  return elm.children.some(({ props }: { props: any }) => {
...
function hasContentVirtual(
  ...
  return (
    ...
    (!noRecursion &&
      elm.children.some(
        (child: any) =>
          child.actualNode.nodeType === 1 && hasContentVirtual(child)
      ))
  );
}
