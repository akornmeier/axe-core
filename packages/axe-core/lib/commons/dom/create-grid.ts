...
interface NegativeIndexArray<T> extends Array<T> {
  _negativeIndex?: number;
}
...
export default function createGrid(
  ...
  if (!parentVNode) {
    let vNode: any = getNodeFromTree(document.documentElement);
    if (!vNode) {
      vNode = new VirtualNode(
        document.documentElement as Node & Element,
        undefined,
        undefined
      );
    }
  ...
  let node: Node | null = parentVNode
    ? treeWalker.nextNode()
    : treeWalker.currentNode;
  while (node) {
    let vNode: any = getNodeFromTree(node);

    if (vNode && vNode.parent) {
      parentVNode = vNode.parent;
    }
    // Elements with an assigned slot need to be a child of the slot element
    else if ((node as Element).assignedSlot) {
      parentVNode = getNodeFromTree((node as Element).assignedSlot);
    }
    ...
    else if ((node as Element).parentElement) {
      parentVNode = getNodeFromTree((node as Element).parentElement);
    } else if (node.parentNode && getNodeFromTree(node.parentNode)) {
      parentVNode = getNodeFromTree(node.parentNode);
    }

    if (!vNode) {
      vNode = new (axe as any).VirtualNode(node, parentVNode);
    }
    ...
    if (
      rect.width !== 0 &&
      rect.height !== 0 &&
      isVisibleOnScreen(node as Element)
    ) {
    ...
    if (isShadowRoot(node as Element)) {
      createGrid((node as Element).shadowRoot!, grid, vNode);
    }
 ...
 * @return {Boolean}
 */
function isStackingContext(vNode: any, parentVNode: any): boolean {
 ...
 * @return {Boolean}
 */
function isFlexOrGridContainer(vNode: any): boolean {
  if (!vNode) {
    return false;
  }
 ...
 * @return {Number[]}
 */
function createStackingOrder(
  vNode: any,
  parentVNode: any,
  treeOrder: number
): StackingContext[] {
  const stackingOrder: StackingContext[] = parentVNode._stackingOrder.slice();
 ...
 */
function createStackingContext(
  stackLevel: number,
  treeOrder: number,
  vNode: any | null
): StackingContext {
 ...
 * @return {Number|null}
 */
function getStackLevel(vNode: any, parentVNode: any): number | null {
 ...
 * @return {Number|'auto'}
 */
function getRealZIndex(vNode: any, parentVNode: any): string {
  const position = vNode.getComputedStylePropertyValue('position');
 ...
 */
function findScrollRegionParent(vNode: any, parentVNode: any): any | null {
  let scrollRegionParent: any | null = null;
  const checkedNodes: any[] = [vNode];
  ...
  checkedNodes.forEach(
    (virtualNode: any) => (virtualNode._scrollRegionParent = scrollRegionParent)
  );
 ...
 * @param {VirtualNode}
 */
function addNodeToGrid(grid: Grid, vNode: any): void {
  const overflowHiddenNodes = getOverflowHiddenAncestors(vNode);

  vNode.clientRects.forEach((clientRect: DOMRect) => {
    // ignore any rects that are outside the bounds of overflow hidden ancestors
    const visibleRect = overflowHiddenNodes.reduce(
      (rect: DOMRect | null, overflowNode: any) => {
        return (
    ...
    grid.loopGridPosition(gridRect, (gridCell: any[]) => {
...
class Grid {
  container: any | null;
  cells: NegativeIndexArray<NegativeIndexArray<any[]>>;
  boundaries?: DOMRect;

  constructor(container: any | null = null) {
   ...
   */
  getCellFromPoint({ x, y }: { x: number; y: number }): any[] {
    ...
    const row =
      (this.cells as any)[rowIndex - (this.cells as any)._negativeIndex] ?? [];
  ...
  loopGridPosition(
    ...
    callback: (gridCell: any[], position: GridPosition) => void
  ): void {
    const { left, right, top, bottom } = gridPosition;
    let pos: DOMRect = gridPosition;
    if (this.boundaries) {
      pos = getBoundingRect(this.boundaries, gridPosition);
    }
    this.boundaries = pos;

    loopNegativeIndexMatrix(
      this.cells as any,
      top,
      bottom,
      (gridRow: any, row: number) => {
        loopNegativeIndexMatrix(
          gridRow,
          left,
          right,
          (gridCell: any, col: number) => {
...
function loopNegativeIndexMatrix(
  matrix: any,
  start: number,
  end: number,
