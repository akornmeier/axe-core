 ...
 * @return {DOMRect[]}
 */
function getTargetRects(vNode: any): DOMRect[] {
  const nodeRect = vNode.boundingClientRect;
  const overlappingVNodes = findNearbyElms(vNode).filter((vNeighbor: any) => {
    return (
  ...
  const obscuringRects = overlappingVNodes.map(
    ({ boundingClientRect: rect }: { boundingClientRect: DOMRect }) => rect
  );
