...
const allowedDisabledNodeNames: string[] = [
  'button',
  'command',
  'fieldset',
  'keygen',
...
function isDisabledAttrAllowed(nodeName: string): boolean {
 ...
 */
function focusDisabled(el: any): boolean {
  const { vNode }: { vNode: any } = nodeLookup(el);
  ...
  ancestors.forEach(
    (ancestor: any) => (ancestor._inDisabledFieldset = fieldsetDisabled)
  );
  if (fieldsetDisabled) {
    return true;
  }
