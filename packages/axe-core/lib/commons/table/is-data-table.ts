 ...
 */
function isDataTable(node: HTMLTableElement): boolean {
  const role = getExplicitRole(node) as string;
  ...
  let bgColor: string | undefined, bgImage: string | undefined;
  for (let rowIndex = 0; rowIndex < rowLength; rowIndex++) {
    row = node.rows[rowIndex]!;
    if (
      bgColor &&
  ...
  const coords = getElementCoordinates(node) as { width: number };
  const viewport = getViewportSize(window) as { width: number };
  if (coords.width > viewport.width * 0.95) {
    return false;
  }
