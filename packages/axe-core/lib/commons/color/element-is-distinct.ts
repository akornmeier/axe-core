 ...
 * @return {Array}
 */
function _getFonts(style: CSSStyleDeclaration): string[] {
  return style
 ...
 * @return {Boolean}
 */
function elementIsDistinct(
  node: HTMLElement,
  ancestorNode: HTMLElement
): boolean {
  ...
  let hasStyle = [
    ...
    'font-style',
    'font-size'
  ].reduce((result: boolean, cssProp: string) => {
    return (
      result ||
