...
interface StackingContext {
  vNode: any;
  ancestor?: StackingContext | undefined;
  opacity: number;
  bgColor: Color;
...
interface StackingContextColorResult {
  color: Color;
  blendMode?: string | undefined;
}
...
export function getStackingContext(
  ...
  const virtualNode = getNodeFromTree(elm) as any;
  if (virtualNode._stackingContext) {
    return virtualNode._stackingContext;
  }

  const stackingContext: StackingContext[] = [];
  const contextMap = new Map<unknown, StackingContext>();
  elmStack = elmStack ?? (getBackgroundStack(elm) as Element[]) ?? [];

  elmStack.forEach((bgElm: Element) => {
    const bgVNode = getNodeFromTree(bgElm) as any;
    ...
    /*
      ...
      a stacking context
     */
    const stackingOrder = bgVNode._stackingOrder.filter(
      ({ vNode }: { vNode: unknown }) => !!vNode
    );

    // create a stacking context for each node in the stacking order
    stackingOrder.forEach(({ vNode }: { vNode: unknown }, index: number) => {
 ...
 * @return {Object}
 */
export function stackingContextToColor(
  context: StackingContext
): StackingContextColorResult {
  ...
  const sourceColor = context.descendants.reduce(
    ...
    createStackingContext() as unknown as Color
  );
 ...
 * @return {Color}
 */
function reduceToColor(
  backdropContext: Color | StackingContext,
  sourceContext: StackingContext
): Color {
  let backdrop: Color;
...
function createStackingContext(
  vNode?: any,
  ancestorContext?: StackingContext
): StackingContext {
  return {
    vNode: vNode,
 ...
 * @return {Object}
 */
function addToStackingContext(
  contextMap: Map<unknown, StackingContext>,
  vNode: unknown,
  ancestorVNode: unknown
): StackingContext {
 ...
 * @return {Color}
 */
function getOwnBackgroundColor(vNode: any): Color {
