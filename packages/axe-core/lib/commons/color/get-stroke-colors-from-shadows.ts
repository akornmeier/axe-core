...
export default function getStrokeColorsFromShadows(
  parsedShadows: ParsedShadow[],
  { ignoreEdgeCount = false }: { ignoreEdgeCount?: boolean | undefined } = {}
): Color[] | null {
  const shadowMap = getShadowColorsMap(parsedShadows);
  const shadowsByColor: ShadowsByColor[] = Object.entries(shadowMap).map(
    ([colorStr, sides]) => {
      const edgeCount = edges.filter(
        side => (sides as ShadowSides)[side]!.length !== 0
      ).length;
      return { colorStr, sides: sides as ShadowSides, edgeCount };
    }
  );
  ...
  return shadowsByColor
    ...
    .filter((shadow): shadow is Color => shadow !== null);
}

/**
 * Create a map of colors to the sides they are on
 */
function getShadowColorsMap(
  parsedShadows: ParsedShadow[]
): Record<string, ShadowSides> {
  const colorMap: Record<string, ShadowSides> = {};
  for (const { colorStr, pixels } of parsedShadows) {
    colorMap[colorStr] ??= { top: [], right: [], bottom: [], left: [] };
    const borders = colorMap[colorStr]!;
    const offsetX = pixels[0] ?? 0;
    const offsetY = pixels[1] ?? 0;
...
function shadowGroupToColor({
  ...
  edges.forEach(edge => {
    ...
    isSolid &&= sides[edge]!.every(
      (offset: number) => offset > OPAQUE_STROKE_OFFSET_MIN_PX
    );
  });
