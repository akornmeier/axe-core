 ...
 * @return {Array}
 */
export default function getBackgroundStack(node: Element): Element[] | null {
  const stacks: Element[][] = getTextElementStack(node).map((stack: any) => {
  ...
  for (let index = 0; index < stacks.length; index++) {
    ...
    if (index !== 0 && !shallowArraysEqual(stack ?? null, stacks[0]!)) {
 ...
 * @returns {Array}
 */
function sortPageBackground(elmStack: Element[]): Element[] {
 ...
 */
function shallowArraysEqual<T>(a: T[] | null, b: T[] | null): boolean {
  if (a === b) {
    return true;
  }
  if (a === null || b === null) {
