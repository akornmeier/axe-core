...
type ImplicitRoleValue =
  | string
  | ((vNode: AbstractVirtualNode) => string | null | undefined);
...
const implicitHtmlRoles: Record<string, ImplicitRoleValue> = {
  a: (vNode: AbstractVirtualNode): string | null => {
    return vNode.hasAttr('href') ? 'link' : null;
  },
  area: (vNode: AbstractVirtualNode): string | null => {
    return vNode.hasAttr('href') ? 'link' : null;
  },
  article: 'article',
  aside: (vNode: AbstractVirtualNode): string | null => {
    if (
      closest(
        vNode.parent as unknown as Record<string, unknown>,
  ...
  fieldset: 'group',
  figure: 'figure',
  footer: (vNode: AbstractVirtualNode): string | null => {
  ...
  form: (vNode: AbstractVirtualNode): string | null => {
  ...
  h5: 'heading',
  h6: 'heading',
  header: (vNode: AbstractVirtualNode): string | null => {
    const sectioningElement = closest(
      vNode as unknown as Record<string, unknown>,
      getSectioningContentPlusMainSelector()
    );
  ...
  hr: 'separator',
  img: (vNode: AbstractVirtualNode): string => {
    ...
    return emptyAlt &&
      !hasGlobalAria &&
      !isFocusable(vNode as unknown as HTMLElement)
      ? 'presentation'
      : 'img';
  },
  input: (vNode: AbstractVirtualNode): string => {
    ...
    if (vNode.hasAttr('list')) {
      const listElement = (
        idrefs(
          (vNode as unknown as { actualNode: HTMLElement }).actualNode,
          'list'
        ) as HTMLElement[]
      ).filter((node: HTMLElement | null) => !!node)[0];
      suggestionsSourceElement =
        !!listElement && listElement.nodeName.toLowerCase() === 'datalist';
    }

    switch ((vNode.props as Record<string, unknown>).type) {
      case 'checkbox':
        return 'checkbox';
      case 'number':
        return 'spinbutton';
  ...
  search: 'search',
  section: (vNode: AbstractVirtualNode): string | null => {
    return hasAccessibleName(vNode) ? 'region' : null;
  },
  select: (vNode: AbstractVirtualNode): string => {
  ...
  table: 'table',
  tbody: 'rowgroup',
  td: (vNode: AbstractVirtualNode): string => {
    const table = closest(vNode as unknown as Record<string, unknown>, 'table');
  ...
  textarea: 'textbox',
  tfoot: 'rowgroup',
  th: (vNode: AbstractVirtualNode): string | undefined => {
