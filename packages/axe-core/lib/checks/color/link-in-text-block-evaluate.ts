...
function getContrast(color1: any, color2: any): number {
...
function linkInTextBlockEvaluate(
  ...
  while (
    parentBlock &&
    parentBlock.nodeType === 1 &&
    !isBlock(parentBlock as HTMLElement)
  ) {
  ...
  const parentColor = getForegroundColor(parentBlock as HTMLElement);
  const nodeBackgroundColor = getBackgroundColor(node);
  const parentBackgroundColor = getBackgroundColor(parentBlock as HTMLElement);

  // Compute contrasts, giving preference to foreground color and doing as little work as possible
  let textContrast: number | undefined =
  ...
  let backgroundContrast: number | undefined =
