...
export default function linkInTextBlockStyleEvaluate(
  this: any,
  node: HTMLElement
): boolean | undefined {
  if (isBlock(node)) {
  ...
  if (elementIsDistinct(node, parentBlock as HTMLElement)) {
    return true;
  }
...
function hasPseudoContent(node: HTMLElement): boolean {
