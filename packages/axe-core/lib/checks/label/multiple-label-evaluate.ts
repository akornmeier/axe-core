...
function multipleLabelEvaluate(
  ...
  let labels: any[] = Array.from(root.querySelectorAll(`label[for="${id}"]`));

  if (labels.length) {
    // filter out CSS hidden labels because they're fine
    labels = labels.filter((label: any) => !isHiddenForEveryone(label));
  }

  while (parent) {
    if (
      (parent as HTMLElement).nodeName.toUpperCase() === 'LABEL' &&
  ...
  if (labels.length > 1) {
    const ATVisibleLabels = labels.filter((label: any) =>
