...
export default function tdHeadersAttrEvaluate(
  this: any,
  node: HTMLTableElement
): boolean | undefined {
  const cells: HTMLTableCellElement[] = [];
  const cellRoleById: Record<string, string> = {};
  ...
  const badCells: Record<string, Set<HTMLTableCellElement>> = {
    [selfRef as string]: new Set(),
    [notInTable as string]: new Set(),
    [notTh as string]: new Set(),
    [emptyHdrs as string]: new Set()
  };
  cells.forEach(cell => {
    ...
    if (!headersAttr) {
      badCells[emptyHdrs as string]!.add(cell);
      return;
    }
