...
export default function ariaRequiredChildrenEvaluate(
  ...
  const unallowed = ownedRoles.filter(
    ({ role, vNode }: any) =>
      vNode.props.nodeType === 1 && !required.includes(role)
  );

  if (unallowed.length) {
    this.relatedNodes(unallowed.map(({ vNode }: any) => vNode));
    this.data({
      messageKey: 'unallowed',
      values: unallowed
        .map(({ vNode, attr }: any) => getUnallowedSelector(vNode, attr))
        .filter(
          (selector: string, index: number, array: string[]) =>
            array.indexOf(selector) === index
        )
        .join(', ')
    });
    return false;
 ...
 */
function getOwnedRoles(virtualNode: any, required: string[]): any[] {
  let vNode: any;
  const ownedRoles: any[] = [];
 ...
 */
function hasRequiredChildren(required: string[], ownedRoles: any[]): boolean {
  return ownedRoles.some(({ role }: any) => role && required.includes(role));
}
 ...
 * @return {String|null}
 */
function getGlobalAriaAttr(vNode: any): string | undefined {
  return getGlobalAriaAttrs().find((attr: string) => vNode.hasAttr(attr));
}
 ...
 * @return {String}
 */
function getUnallowedSelector(vNode: any, attr?: string): string {
 ...
 * @returns {Boolean}
 */
function isContent({ vNode }: any): boolean {
  if (vNode.props.nodeType === 3) {
