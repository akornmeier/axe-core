...
export default function ariaProhibitedAttrEvaluate(
  ...
  const prohibited = prohibitedList.filter((attrName: string) => {
...
function listProhibitedAttrs(
  ...
  const roleSpec = standards.ariaRoles[role as string];
  if (roleSpec) {
...
const getClosestAncestorRoleType = memoize(
  function getClosestAncestorRoleTypeMemoized(vNode: any): string | undefined {
    if (!vNode) {
      return;
    }
    ...
    if (role) {
      return getRoleType(role) ?? undefined;
    }
