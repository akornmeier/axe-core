 ...
 */
export default function ariaErrormessageEvaluate(
  this: any,
  node: HTMLElement,
  options: any,
  virtualNode: any
  ...
  const self = this;
  function validateAttrValue(attr: string): boolean | undefined {
    if (attr.trim() === '') {
    ...
    if (errormessageTokens.length > 1) {
      self.data({ messageKey: 'unsupported', values: errormessageTokens });
      return false;
    }
    let idref: any;
    ...
    if (idref) {
      ...
      return (
        ...
        errormessageTokens.some((token: string) =>
          describedbyTokens.includes(token)
        )
      );
    }
  ...
  if (options.indexOf(errorMessageAttr) === -1 && hasAttr) {
    ...
    return validateAttrValue(errorMessageAttr);
  }
