 ...
 */
const VALID_TAG_NAMES_FOR_SCROLLABLE_REGIONS: Record<string, boolean> = {
  ARTICLE: true,
  ASIDE: true,
  NAV: true,
  SECTION: true
 ...
 * appropriate for scrollable elements found in the focus order.
 */
const VALID_ROLES_FOR_SCROLLABLE_REGIONS: Record<string, boolean> = {
  alert: true,
  alertdialog: true,
  application: true,
  article: true,
 ...
 *		 region.
 */
function validScrollableTagName(node: HTMLElement): boolean {
 ...
 *		 region.
 */
function validScrollableRole(node: HTMLElement, options: any): boolean {
 ...
 */
function validScrollableSemanticsEvaluate(
  node: HTMLElement,
  options: any
): boolean {
