...
function focusableDisabledEvaluate(
  this: any,
  node: HTMLElement,
  ...
  const relatedNodes = tabbableElements.filter((vNode: any) => {
    return elementsThatCanBeDisabled.includes(vNode.props.nodeName);
  });

  this.relatedNodes(relatedNodes.map((vNode: any) => vNode.actualNode));

  if (relatedNodes.length === 0 || isModalOpen()) {
    return true;
  }

  return relatedNodes.every((vNode: any) => {
